...
class Formula(PyFunction):
    ''' Function from PyRational to PyRational
    '''
    def __init__(self, variables, formula):
        assert isinstance(variables, PySet)
        assert isinstance(formula, string)
    ...
    def calculate(self, value):
        assert len(list(variables.generator())) == len(value)
        for val in value.keys():
            formula = formula.replace(val, value[val])
            
        return evaluate(tokenize(formula))
...
if __name__ == '__main__':
    ...
    eq12 = '3/4/2'
    print(tokenize(eq12))
    print(find_chunk(tokenize(eq12), 1))
    eq13 = '(3/4)/2'
    eq14 = '3/(4/2)'
    eq15 = '1+2/3'
